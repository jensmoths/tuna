...
from fcs_bridge.blackbox_download import BLACKBOX_HEADER, transfer_blackbox_log_from_bridge, download_msc_raw
...
class FcsTransferTests(unittest.TestCase):
    def test_download_msc_raw_downloads_in_chunks_and_records_progress(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("fcs_bridge.blackbox_download._read_msc_raw_range", side_effect=fake_read):
    ...
    def test_transfer_discovers_size_before_triggering_msc(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("fcs_bridge.blackbox_download.read_bridge_status") as status:
                ...
                with patch("fcs_bridge.blackbox_download.discover_blackbox_transfer_size", return_value=1234) as discover:
                    with patch("fcs_bridge.blackbox_download.trigger_msc_mode", return_value="msc"):
                        with patch("fcs_bridge.blackbox_download.wait_for_msc_raw", return_value=status.return_value):
                            with patch("fcs_bridge.blackbox_download.download_msc_raw") as download:
    ...
    def test_transfer_prefers_msc_file_when_available(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("fcs_bridge.blackbox_download.read_bridge_status") as status:
                ...
                with patch("fcs_bridge.blackbox_download.download_preferred_msc_file", return_value=file_payload) as file_download:
                    with patch("fcs_bridge.blackbox_download.download_msc_raw") as raw_download:
    ...
    def test_transfer_falls_back_to_raw_when_msc_file_unavailable(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("fcs_bridge.blackbox_download.read_bridge_status") as status:
                ...
                status.return_value.msc_mounted = True
                status.return_value.text = "msc_raw=1 msc_mounted=1"
                with patch("fcs_bridge.blackbox_download.download_preferred_msc_file", return_value=None):
                    with patch("fcs_bridge.blackbox_download.download_msc_raw") as raw_download:
