...
class PiSupervisorWebTests(unittest.TestCase):
    ...
    def test_loop_page_starts_pi_rpc_tuning_agent(self):
        ...
        self.assertIn("include `--loop-id", sent)
        self.assertIn("stop for Operator confirmation", sent)
        self.assertIn("Do not choose no-change solely because the evidence is fixture-sourced", sent)
        self.assertIn("include only the settings you intend to change", sent)
        self.assertIn("do not copy unchanged FC settings", sent)
